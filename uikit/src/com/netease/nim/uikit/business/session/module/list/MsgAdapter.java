...
public class MsgAdapter extends BaseMultiItemFetchLoadAdapter<IMMessage, BaseViewHolder> {
    ...
    private MyMsgListener msgListener;
    ...
    MsgAdapter(RecyclerView recyclerView, List<IMMessage> data, Container container) {
        ...
        HashMap<Class<? extends MsgAttachment>, Class<? extends MsgViewHolderBase>> viewHolders = MsgViewHolderFactory.getViewHolders();
        for (Class<? extends MsgAttachment> attachment : viewHolders.keySet()) {
            viewType++;
            if (CustomAttachment.class.isAssignableFrom(attachment)) {
                addItemType(viewType, R.layout.nim_message_item2, viewHolders.get(attachment));
                holder2ViewType.put(viewHolders.get(attachment), viewType);
            } else {
                addItemType(viewType, R.layout.nim_message_item, viewHolders.get(attachment));
                holder2ViewType.put(viewHolders.get(attachment), viewType);
            }
        }

        List<Class<? extends MsgViewHolderBase>> holders = MsgViewHolderFactory.getOtherViewHolders();
    ...
    public MyMsgListener getMsgListener() {
        return msgListener;
    }

    public void setMsgListener(MyMsgListener msgListener) {
...
//            notifyDataSetChanged(); // 可以不要！！！
    ...
    public void updateShowTimeItem(List<IMMessage> items, boolean fromStart, boolean update) {
        ...
        if (msgListener != null) msgListener.onCall(anchor);
