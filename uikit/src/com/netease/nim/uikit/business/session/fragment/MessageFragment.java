...
public class MessageFragment extends TFragment implements ModuleProxy {
    ...
    protected String intenviewID = null;
    protected String positionID = null;
    protected String intenviewName = null;

    public MessageFragment() {
    }

    @SuppressLint("ValidFragment")
    public MessageFragment(String intenviewID, String positionID, String intenviewName, MyMsgListener msgListener, MyMsgSendListener myMsgSendListener) {
    ...
    public void setFlg(String intenviewID, String positionID, String intenviewName) {
        this.intenviewID = intenviewID;
        this.positionID = positionID;
        this.intenviewName = intenviewName;
    }
    ...
    protected MyMsgListener msgListener;
    protected MyMsgSendListener myMsgSendListener;
    ...
    protected void parseIntent() {
        ...
        if (messageListPanel == null) {
            messageListPanel = new MessageListPanelEx(container, rootView, anchor, false, false, msgListener);
        } else {
    ...
    protected void onMessageIncoming(List<IMMessage> messages) {
     ...
     * 已读回执观察者
     */
    protected Observer<List<MessageReceipt>> messageReceiptObserver = new Observer<List<MessageReceipt>>() {
        @Override
...
//                    messageListPanel.refreshMessageList();
        ...
        if (myMsgSendListener != null) myMsgSendListener.onSend();
        if (aitManager != null) {
            aitManager.reset();
        }
        return true;
    ...
    public boolean sendMessage2(IMMessage message) {
        //data: { intenviewID: 求职意向id,  positionID: 职位ID，intenviewName :意向职位名称, ownMebile: 手机号}
        if (positionID == null) return false;
        Map map = new HashMap();
        map.put("intenviewID", intenviewID);
    ...
    protected void sendFailWithBlackList(int code, IMMessage msg) {
    ...
    protected void appendTeamMemberPush(IMMessage message) {
        if (aitManager == null) {
            return;
        }
    ...
    protected IMMessage changeToRobotMsg(IMMessage message) {
        if (aitManager == null) {
            return message;
        }
    ...
    protected boolean isChatWithRobot() {
        return NimUIKitImpl.getRobotInfoProvider().getRobotByAccount(sessionId) != null;
    }

    protected void appendPushConfig(IMMessage message) {
